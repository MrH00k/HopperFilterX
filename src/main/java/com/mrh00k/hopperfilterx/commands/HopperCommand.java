...
public class HopperCommand implements CommandExecutor, TabCompleter {
  ...
  public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
    try {
      if (sender instanceof Player && !sender.hasPermission("hopperfilterx.give")) {
      ...
      if (args.length < 3) {
        ...
        if (removed) {
          ...
          sender.sendMessage(
              MessageManager.getInstance()
                  .getMessage(
                      "command.remove-not-found", "player", targetPlayerName, "uuid", "ALL"));
        }
  ...
  private void handleListCommand(CommandSender sender, String[] args) {
    try {
      if (!(sender.hasPermission("hopperfilterx.list") || sender.isOp())) {
        if (sender instanceof Player) {
          SoundManager.playErrorSound((Player) sender);
        }
